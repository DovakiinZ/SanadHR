...
import { ArrowRight, Building2, Loader2, Save, Upload, FileText } from "lucide-react";
...
export default function CompanySettingsPage() {
  ...
  const canEdit = has("Platform.CompanyConfig.Edit") || has("Settings.Edit");
  ...
  const save = async () => {
    ...
    setSaving(true);
    try {
      const saved = await saveCompanyProfile(p);
      setP(saved);
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">المصدر الموحّد لبيانات الشركة — تُستخدم تلقائياً في المستندات والتقارير والطباعة</p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        ...
        <ImageField label="ختم الشركة" icon={FileText} url={p.stampUrl} disabled={!canEdit} onUploaded={(u) => set("stampUrl", u)} />
      </div>
...
function ImageField({ label, icon: Icon, url, onUploaded, disabled }: { label: string; icon: ElementType; url?: string | null; onUploaded: (u: string) => void; disabled?: boolean }) {
  ...
  const onFile = async (f?: File) => {
    if (!f) return;
    setUploading(true);
    try { const r = await uploadFile(f, "company"); onUploaded(r.url); toast.success("تم الرفع"); }
    catch { toast.error("تعذر الرفع"); }
    finally { setUploading(false); }
  };
  return (
    <div className="flex items-center gap-4 border border-border bg-card p-4">
      <div className="flex h-20 w-20 shrink-0 items-center justify-center border border-border bg-secondary">
      ...
      <div className="min-w-0">
        ...
        {!disabled && (
          <button onClick={() => ref.current?.click()} disabled={uploading} className="mt-2 inline-flex h-9 items-center gap-2 border border-border px-3 text-sm hover:bg-muted disabled:opacity-50">
